...
-- File: MoffatBayMarinaDB_V1-8-0.sql
-- Purpose: Builds the complete database FROM SCRATCH at version 1.8.0 -
--          the state it is in today. This single file replaces the old
--          two-step path of running MoffatBayMarinaDB_V1-7-0.sql and then
--          the V1-8-0 update script. Those two files still exist, under
--          Legacy/Week6/, as the record of how the schema got here; this
--          is the file to actually run.
--          (V1-7-0.sql was itself a consolidation of an earlier four-step
--          path, and V1-4-0.sql of a five-step path before that - see
--          Legacy/Week5/ and Legacy/Week4/ for that older history.)
--
--          Folded in, relative to V1-0-0:
--            1.1.0  Customer.failedLoginAttempts, accountLocked,
--                   phoneCountryCode, streetAddress2
--            1.2.0  Customer.country
--            1.3.0  Boat.regState dropped; regNumber nullable and unique
--                   on its own
--            1.4.0  Reservation.reservationStatus
--            1.5.0  Adds Rate (per-foot slip rent + flat electric fee);
--                   repairs Boat.regNumber values doubled by V1-3-0 (a
--                   no-op here - this file seeds the correct values
--                   directly)
--            1.6.0  Reservation.electricalHookup
--            1.7.0  Dock.dockDescription rewritten as a three-line
--                   name/compass-label/landmark stat block
--            1.8.0  Every seeded Employee and Customer password reset to
--                   one that meets the site's password rules (a no-op
--                   here - this file seeds the compliant passwords
--                   directly, rather than seeding the old ones and
--                   UPDATE-ing them afterwards)
--
...
--          INSERT (its employeeID / customerID on a fresh build), e.g.
--          elena.marsh@example.com -> MoffatMarsh01!. Each one satisfies
--          Utils.PASSWORD_PATTERN and formValidation.js's PASSWORD_RULES:
--          at least 10 characters, one uppercase, one lowercase, one
...
-- Date: 2026-09-20
-- =============================================================================
-- Run this script as a MySQL admin/root user (e.g. `mysql -u root -p < MoffatBayMarinaDB_V1-8-0.sql`).
...
-- The full lineage, so a database built from this one file is
-- indistinguishable from one built by running V1-0-0 and every update since.
-- Dates are the dates each version was authored, not the date this script
-- runs - CURDATE() would claim all seven happened today, which is untrue.
-- currentVersion.sql reads the last row, so this instance reports 1.8.0.
INSERT INTO DatabaseVersion (version, appliedDate, description) VALUES
    ('1.0.0', '2026-08-28', 'Initial consolidated single-file release: Dock, Employee, Customer, Boat, SlipSize, WaitList, BoatOwnership, Slip, Contact, Reservation, TerminationNotice.'),
    ('1.1.0', '2026-08-31', 'Customer: adds failedLoginAttempts and accountLocked for the login lockout flow, plus phoneCountryCode and streetAddress2 for the registration form.'),
    ('1.2.0', '2026-09-04', 'Customer: adds country (US/CA/OTHER), driving the Registration page''s foreign-registration badge and the Boat Registration State/Province field.'),
    ('1.3.0', '2026-09-06', 'Boat: drops regState; regNumber now stores the full registration number including the state/province prefix, and is nullable.'),
    ('1.4.0', '2026-09-06', 'Reservation: adds reservationStatus (designed in the ERD but missing from V1-0-0 creation script).'),
    ('1.5.0', '2026-09-07', 'Adds the Rate table (per-foot slip rent and flat electric fee) and repairs Boat.regNumber values doubled by V1-3-0.'),
    ('1.6.0', '2026-09-10', 'Adds Reservation.electricalHookup (BOOLEAN NOT NULL DEFAULT FALSE) - records whether a reservation includes the optional electric hookup.'),
...
CREATE TABLE Dock (
    ...
    dockDescription VARCHAR(255) COMMENT 'A customer facing brief description of the dock, as three CHAR(10)-separated lines: name, compass label, nearest landmark. reservation.css needs white-space: pre-line on .dock-card__desc for the line breaks to show.'
);

-- Three-line name/compass-label/landmark stat block, matching marina_a.png:
-- the Office & Restaurant marker sits measurably closer to Dock B than Dock
-- C, and the Fuel Dock marker sits past Dock C's end.
INSERT INTO Dock (dockNumber, dockDescription) VALUES
    ('A', CONCAT('Linear Dock A', CHAR(10), 'Eastern Dock', CHAR(10), 'Closest to the Ship Store')),
    ('B', CONCAT('Linear Dock B', CHAR(10), 'Central Dock', CHAR(10), 'Closest to the Office & Restaurant')),
    ('C', CONCAT('Linear Dock C', CHAR(10), 'Western Dock', CHAR(10), 'Closest to the Fueling Station'));
...
INSERT INTO Employee (firstName, lastName, email, passwordHash, phone, jobTitle, hireDate) VALUES
    ('Maria', 'Alvarez', 'maria.alvarez@moffatbaymarina.com', SHA2('MoffatAlvarez01!', 256), '360-555-0101', 'Marina Manager', '2019-03-01'),
    ('Tom', 'Whitfield', 'tom.whitfield@moffatbaymarina.com', SHA2('MoffatWhitfield02!', 256), '360-555-0102', 'Dockmaster', '2020-06-15'),
    ('Priya', 'Nair', 'priya.nair@moffatbaymarina.com', SHA2('MoffatNair03!', 256), '360-555-0103', 'Customer Service Rep', '2022-01-10'),
    ('Derek', 'Simmons', 'derek.simmons@moffatbaymarina.com', SHA2('MoffatSimmons04!', 256), '360-555-0104', 'Maintenance Technician', '2021-09-01'),
    ('Lena', 'Cho', 'lena.cho@moffatbaymarina.com', SHA2('MoffatCho05!', 256), '360-555-0105', 'Billing Clerk', '2023-04-20'),
    ('Carlos', 'Reyes', 'carlos.reyes@moffatbaymarina.com', SHA2('MoffatReyes06!', 256), '360-555-0106', 'Fueling Station Manager', '2020-11-02'),
    ('Sophie', 'Bennett', 'sophie.bennett@moffatbaymarina.com', SHA2('MoffatBennett07!', 256), '360-555-0107', 'Shop Manager', '2022-07-18');
...
CREATE TABLE Customer (
    ...
    -- Added in 1.1.0. Kept at the end of the table so this matches the
    -- column order an ALTER-built database ends up with.
    ...
    phoneCountryCode VARCHAR(3) NOT NULL DEFAULT '1'
...
VALUES
    ('Elena', 'Marsh', 'elena.marsh@example.com', SHA2('MoffatMarsh01!', 256), '360-555-0201', '412 Harborview Dr', 'Moffat Bay', 'WA', '98110', '2025-03-14'),
    ('Desmond', 'Okafor', 'desmond.okafor@example.com', SHA2('MoffatOkafor02!', 256), '360-555-0202', '87 Tidewater Ln', 'Port Grayson', 'WA', '98221', '2025-06-02'),
    ('Yuki', 'Tanaka', 'yuki.tanaka@example.com', SHA2('MoffatTanaka03!', 256), '360-555-0203', '1550 Anchorage Way', 'Moffat Bay', 'WA', '98110', '2025-09-19'),
    ('Rosa', 'Delgado', 'rosa.delgado@example.com', SHA2('MoffatDelgado04!', 256), '360-555-0204', '23 Gull Point Rd', 'Sable Cove', 'OR', '97103', '2026-01-08'),
    ('Arthur', 'Penhale', 'arthur.penhale@example.com', SHA2('MoffatPenhale05!', 256), '360-555-0205', '9 Old Pier St', 'Moffat Bay', 'WA', '98110', '2026-02-27'),
    ('Nadia', 'Brennan', 'nadia.brennan@example.com', SHA2('MoffatBrennan06!', 256), '360-555-0206', '744 Saltgrass Ave', 'Port Grayson', 'WA', '98221', '2026-05-30'),
    ('Kenneth', 'Pearson', 'kenneth.pearson@example.com', SHA2('MoffatPearson07!', 256), '360-555-0300', '30 Driftwood Ave', 'Moffat Bay', 'WA', '98110', '2025-08-31'),
    ('William', 'Dunmore', 'william.dunmore@example.com', SHA2('MoffatDunmore08!', 256), '360-555-0301', '109 Cape Ln', 'Port Grayson', 'WA', '98221', '2026-03-23'),
    ('Robert', 'Foster', 'robert.foster@example.com', SHA2('MoffatFoster09!', 256), '360-555-0302', '100 Beacon Ave', 'Sable Cove', 'OR', '97103', '2026-06-16'),
    ('Joshua', 'Foster', 'joshua.foster@example.com', SHA2('MoffatFoster10!', 256), '360-555-0303', '579 Beacon Pl', 'Moffat Bay', 'WA', '98110', '2025-08-28'),
    ('Mark', 'Okonkwo', 'mark.okonkwo@example.com', SHA2('MoffatOkonkwo11!', 256), '360-555-0304', '289 Harbor Rd', 'Port Grayson', 'WA', '98221', '2026-03-23'),
    ('Nancy', 'Blackwood', 'nancy.blackwood@example.com', SHA2('MoffatBlackwood12!', 256), '360-555-0305', '164 Beacon Ct', 'Sable Cove', 'OR', '97103', '2025-04-29'),
    ('Jennifer', 'Rourke', 'jennifer.rourke@example.com', SHA2('MoffatRourke13!', 256), '360-555-0306', '104 Marina Ct', 'Moffat Bay', 'WA', '98110', '2025-10-12'),
    ('Robert', 'Bellweather', 'robert.bellweather@example.com', SHA2('MoffatBellweather14!', 256), '360-555-0307', '475 Cape Ln', 'Port Grayson', 'WA', '98221', '2026-02-06'),
    ('Jennifer', 'Prescott', 'jennifer.prescott@example.com', SHA2('MoffatPrescott15!', 256), '360-555-0308', '305 Bay Ct', 'Sable Cove', 'OR', '97103', '2025-07-30'),
    ('Melissa', 'Sinclair', 'melissa.sinclair@example.com', SHA2('MoffatSinclair16!', 256), '360-555-0309', '51 Shoal Dr', 'Moffat Bay', 'WA', '98110', '2025-04-06'),
    ('Joseph', 'Castillo', 'joseph.castillo@example.com', SHA2('MoffatCastillo17!', 256), '360-555-0310', '394 Driftwood St', 'Port Grayson', 'WA', '98221', '2026-01-23'),
    ('David', 'Tremaine', 'david.tremaine@example.com', SHA2('MoffatTremaine18!', 256), '360-555-0311', '368 Beacon Dr', 'Sable Cove', 'OR', '97103', '2025-03-29'),
    ('Joshua', 'Strand', 'joshua.strand@example.com', SHA2('MoffatStrand19!', 256), '360-555-0312', '180 Cape Ave', 'Moffat Bay', 'WA', '98110', '2025-07-01'),
    ('Sandra', 'Rourke', 'sandra.rourke@example.com', SHA2('MoffatRourke20!', 256), '360-555-0313', '281 Cape Ave', 'Port Grayson', 'WA', '98221', '2025-12-13'),
    ('Stephanie', 'Rivas', 'stephanie.rivas@example.com', SHA2('MoffatRivas21!', 256), '360-555-0314', '62 Shoal Way', 'Sable Cove', 'OR', '97103', '2025-12-04'),
    ('Betty', 'Blackwood', 'betty.blackwood@example.com', SHA2('MoffatBlackwood22!', 256), '360-555-0315', '72 Beacon Ct', 'Moffat Bay', 'WA', '98110', '2025-08-20'),
    ('Carol', 'Corcoran', 'carol.corcoran@example.com', SHA2('MoffatCorcoran23!', 256), '360-555-0316', '410 Current Rd', 'Port Grayson', 'WA', '98221', '2025-10-13'),
    ('William', 'Mercer', 'william.mercer@example.com', SHA2('MoffatMercer24!', 256), '360-555-0317', '767 Cape Dr', 'Sable Cove', 'OR', '97103', '2026-03-29'),
    ('Donna', 'Winslow', 'donna.winslow@example.com', SHA2('MoffatWinslow25!', 256), '360-555-0318', '375 Shoal Rd', 'Moffat Bay', 'WA', '98110', '2026-06-20'),
    ('Ashley', 'Whitmore', 'ashley.whitmore@example.com', SHA2('MoffatWhitmore26!', 256), '360-555-0319', '778 Tide Ln', 'Port Grayson', 'WA', '98221', '2025-06-20'),
    ('Kenneth', 'Sutherland', 'kenneth.sutherland@example.com', SHA2('MoffatSutherland27!', 256), '360-555-0320', '816 Wharf Ln', 'Sable Cove', 'OR', '97103', '2026-02-13'),
    ('Matthew', 'Beaumont', 'matthew.beaumont@example.com', SHA2('MoffatBeaumont28!', 256), '360-555-0321', '484 Breaker Dr', 'Moffat Bay', 'WA', '98110', '2025-01-26'),
    ('Amanda', 'Bellweather', 'amanda.bellweather@example.com', SHA2('MoffatBellweather29!', 256), '360-555-0322', '122 Cape Dr', 'Port Grayson', 'WA', '98221', '2025-12-29'),
    ('Linda', 'Alonso', 'linda.alonso@example.com', SHA2('MoffatAlonso30!', 256), '360-555-0323', '450 Compass St', 'Sable Cove', 'OR', '97103', '2025-01-18'),
    ('George', 'Bellweather', 'george.bellweather@example.com', SHA2('MoffatBellweather31!', 256), '360-555-0324', '274 Breaker Rd', 'Moffat Bay', 'WA', '98110', '2026-06-18'),
    ('Michael', 'Strand', 'michael.strand@example.com', SHA2('MoffatStrand32!', 256), '360-555-0325', '310 Breaker Ave', 'Port Grayson', 'WA', '98221', '2025-06-20'),
    ('Lisa', 'Castellan', 'lisa.castellan@example.com', SHA2('MoffatCastellan33!', 256), '360-555-0326', '170 Cape Way', 'Sable Cove', 'OR', '97103', '2025-12-12'),
    ('Ashley', 'Foster', 'ashley.foster@example.com', SHA2('MoffatFoster34!', 256), '360-555-0327', '119 Marina Dr', 'Moffat Bay', 'WA', '98110', '2025-09-17'),
    ('Patricia', 'Mercer', 'patricia.mercer@example.com', SHA2('MoffatMercer35!', 256), '360-555-0328', '904 Fathom Ln', 'Port Grayson', 'WA', '98221', '2025-04-12'),
    ('George', 'Corcoran', 'george.corcoran@example.com', SHA2('MoffatCorcoran36!', 256), '360-555-0329', '840 Anchor Rd', 'Sable Cove', 'OR', '97103', '2025-05-26'),
    ('Kevin', 'Whitfield', 'kevin.whitfield@example.com', SHA2('MoffatWhitfield37!', 256), '360-555-0330', '974 Cape Rd', 'Moffat Bay', 'WA', '98110', '2025-10-13'),
    ('Kimberly', 'Beaumont', 'kimberly.beaumont@example.com', SHA2('MoffatBeaumont38!', 256), '360-555-0331', '438 Beacon Ave', 'Port Grayson', 'WA', '98221', '2025-11-30'),
    ('Betty', 'Redmond', 'betty.redmond@example.com', SHA2('MoffatRedmond39!', 256), '360-555-0332', '670 Marina St', 'Sable Cove', 'OR', '97103', '2026-06-28'),
    ('Mark', 'Pearson', 'mark.pearson@example.com', SHA2('MoffatPearson40!', 256), '360-555-0333', '258 Shoal Ln', 'Moffat Bay', 'WA', '98110', '2025-12-27'),
    ('Mary', 'Okonkwo', 'mary.okonkwo@example.com', SHA2('MoffatOkonkwo41!', 256), '360-555-0334', '572 Shoal Ave', 'Port Grayson', 'WA', '98221', '2025-01-22'),
    ('John', 'Sharma', 'john.sharma@example.com', SHA2('MoffatSharma42!', 256), '360-555-0335', '651 Tide Ave', 'Sable Cove', 'OR', '97103', '2025-03-25'),
    ('Robert', 'Kowalski', 'robert.kowalski@example.com', SHA2('MoffatKowalski43!', 256), '360-555-0336', '77 Breaker Ave', 'Moffat Bay', 'WA', '98110', '2025-10-27'),
    ('Kevin', 'Corcoran', 'kevin.corcoran@example.com', SHA2('MoffatCorcoran44!', 256), '360-555-0337', '224 Cape Rd', 'Port Grayson', 'WA', '98221', '2026-05-14'),
    ('Jessica', 'Whitfield', 'jessica.whitfield@example.com', SHA2('MoffatWhitfield45!', 256), '360-555-0338', '831 Wharf Ave', 'Sable Cove', 'OR', '97103', '2025-04-21'),
    ('Michael', 'Redmond', 'michael.redmond@example.com', SHA2('MoffatRedmond46!', 256), '360-555-0339', '446 Marina Pl', 'Moffat Bay', 'WA', '98110', '2026-03-11'),
    ('Sandra', 'Bellweather', 'sandra.bellweather@example.com', SHA2('MoffatBellweather47!', 256), '360-555-0340', '60 Gull Way', 'Port Grayson', 'WA', '98221', '2026-03-03'),
    ('George', 'Kowalski', 'george.kowalski@example.com', SHA2('MoffatKowalski48!', 256), '360-555-0341', '824 Gull Ave', 'Sable Cove', 'OR', '97103', '2025-07-30'),
    ('Richard', 'Landry', 'richard.landry@example.com', SHA2('MoffatLandry49!', 256), '360-555-0342', '464 Pier Pl', 'Moffat Bay', 'WA', '98110', '2025-07-21'),
    ('Sarah', 'Marchetti', 'sarah.marchetti@example.com', SHA2('MoffatMarchetti50!', 256), '360-555-0343', '260 Anchor St', 'Port Grayson', 'WA', '98221', '2026-08-01'),
    ('Michael', 'Barrett', 'michael.barrett@example.com', SHA2('MoffatBarrett51!', 256), '360-555-0344', '672 Cape Way', 'Sable Cove', 'OR', '97103', '2025-04-20'),
    ('Edward', 'Mercer', 'edward.mercer@example.com', SHA2('MoffatMercer52!', 256), '360-555-0345', '175 Wharf St', 'Moffat Bay', 'WA', '98110', '2026-05-22');
...
CREATE TABLE Boat (
    ...
    -- regState was dropped in 1.3.0; the owner types the state/province
    -- prefix as part of regNumber itself.
    regNumber VARCHAR(20) NULL
        COMMENT 'Registration number, including the state/province prefix the owner typed (e.g. WN1204JT, or C1234AB for a Canadian Pleasure Craft Licence).',
    ...
    CONSTRAINT uqBoatRegistration UNIQUE (regNumber)
);
...
VALUES
    ('MFT412A3B404', 'Salt Whisper',       'WN1204JT', 'Sailboat',  24.5,  8.2, 2014),
    ('BNT52C7D8188', 'Second Wind',        'WN3391RK', 'Powerboat', 38.0, 13.5, 2019),
    ('CRS63E9F2224', 'Meridian',           'WN0087PL', 'Catamaran', 47.5, 22.0, 2021),
    ('HLM74G1H6360', 'Gullwing',           'OR5540BM', 'Sailboat',  31.0, 10.4, 2008),
    ('SBK85J3K0504', 'Little Bear',        'WN7712DF', 'Powerboat', 22.0,  8.0, 2016),
    (NULL,           'Wandering Albatross','WN0431XG', 'Sailboat',  36.0, 11.0, 1968),
    ('QLCA5GEP8SJY', 'Fair Winds', 'WA2127HM', 'Catamaran', 32.6, 8.6, 1995),
    ('XTKGJSHGKTUP', 'Following Sea', 'WA6617GX', 'Pontoon', 24.3, 6.3, 2001),
    ('CWPTCAXJSL43', 'Tranquility Base', 'WA1158DC', 'Pontoon', 40.5, 9.9, 1994),
    ('PEBVZCYPRGY2', 'Sea Glass', 'WA3532HF', 'Powerboat', 28.4, 8.2, 2011),
    ('XZHTLG0C6QN5', 'Barnacle Bill', 'OR6000HH', 'Sailboat', 30.1, 7.3, 1997),
    ('MWS1XBHSMSCG', 'Tidewater Spirit', 'WA8119MZ', 'Catamaran', 37.7, 9.3, 2012),
    ('JBY3A9NZ3EXW', 'Silver Wake', 'WA3041ZK', 'Center Console', 39.3, 12.5, 2004),
    ('TEG20MV1AVUP', 'Nauti Buoy', 'WA6279QQ', 'Trawler', 39.1, 11.2, 1998),
    ('XCK8XFRVQNKB', 'Windward Bound', 'WA5011RV', 'Sailboat', 21.5, 5.3, 2014),
    ('RNHKAG3QM95D', 'Kingfisher', 'OR2988QE', 'Trawler', 24.6, 6.7, 2018),
    ('TQF64SRT97RT', 'Sea Fever', 'OR2269YK', 'Powerboat', 48.6, 11.9, 2002),
    ('EHNKPE22X52D', 'Ebb and Flow', 'WA7883NU', 'Pontoon', 20.4, 5.6, 1986),
    ('AMK02Q7QT37B', 'Reel Time', 'OR6507XX', 'Trawler', 24.6, 6.7, 1995),
    ('NUUBF3J5MDS0', 'Stormy Petrel', 'WA4467QL', 'Catamaran', 21.5, 6.8, 2009),
    ('CRADYQECBRNB', 'Salty Dog', 'WA4908ER', 'Pontoon', 36.6, 9.2, 1992),
    ('MFVHLVGBV01N', 'Restless Fathom', 'WA4978DY', 'Catamaran', 25.1, 7.1, 2023),
    ('MTPZE8XA27G3', 'Wind Dancer', 'WA6934WQ', 'Pontoon', 24.0, 6.5, 1994),
    ('VTR53TWRFT4R', 'Vagabond', 'WA8613UV', 'Pontoon', 35.1, 9.7, 2009),
    ('RGMSXTTA9NFR', 'Deep Water Dream', 'WA7658RT', 'Powerboat', 28.3, 7.0, 2015),
    ('CKH1RVZ69Y3X', 'Neptunes Grace', 'WA8434JK', 'Catamaran', 36.2, 10.2, 1999),
    ('GGZ6T9UGNUQZ', 'Anchors Away', 'WA5952AY', 'Center Console', 36.3, 10.7, 1993),
    ('YEW7GAU664XM', 'Tidal Grace', 'WA5136RD', 'Sailboat', 18.3, 6.0, 2010),
    ('EEUVFRH2Q904', 'High Tide', 'WA8253KU', 'Trawler', 33.3, 8.9, 2004),
    ('GWGSFLRMELA2', 'Coral Wanderer', 'WA5771BH', 'Catamaran', 35.1, 9.4, 2003),
    ('WUXN3HQKTKED', 'Rip Current', 'WA6039VZ', 'Center Console', 33.2, 10.1, 2003),
    ('JST74FC3WSBF', 'Gale Force', 'OR1339XJ', 'Center Console', 34.2, 8.9, 1987),
    ('JFU37F6Y2XWG', 'Aegean Mist', 'OR3634LP', 'Pontoon', 36.7, 10.3, 2016),
    ('QCLSWH18A52D', 'Second Chance', 'WA9494MV', 'Trawler', 23.0, 6.6, 2013),
    ('KQY7HBRLVA2F', 'Cutlass', 'WA2858QD', 'Pontoon', 18.4, 5.5, 1994),
    ('PRRR5K0N8JET', 'Windswept', 'WA7798LS', 'Catamaran', 38.7, 11.6, 1985),
    ('EQT6YX05WNR0', 'Weekend Warrior', 'OR1715LZ', 'Trawler', 23.4, 5.8, 2009),
    ('RBE8XG4G95AK', 'Northbound', 'WA3529CR', 'Catamaran', 39.4, 10.0, 2006),
    ('LWY7CERUQF3G', 'Horizon Chaser', 'OR2646QF', 'Pontoon', 22.9, 5.7, 2004),
    ('MMPKR92MLMF0', 'Drift Away', 'WA4945RU', 'Powerboat', 28.5, 7.2, 1999),
    ('QKXLE4YV3S5V', 'Northern Star', 'WA4886NU', 'Catamaran', 22.8, 6.4, 2021),
    ('JAUD7UQYQNSJ', 'Gulfstream', 'OR1645KQ', 'Sailboat', 25.4, 7.6, 2022),
    ('LZPMNJZ98TLS', 'Blue Yonder', 'WA8894KZ', 'Catamaran', 23.5, 7.6, 1992),
    ('XZX1ZF1ASH5Z', 'Landfall', 'WA5295UN', 'Pontoon', 25.2, 8.0, 2008),
    ('VHWE5V2HJCCV', 'Pacific Drift', 'WA2902DH', 'Center Console', 23.1, 6.5, 1993),
    ('UZZK2G72TCZP', 'Blue Lagoon', 'OR4868MD', 'Pontoon', 39.9, 10.5, 2008),
    ('NJGH5FPBDXRJ', 'Full Circle', 'WA4362CT', 'Powerboat', 47.8, 12.7, 2022),
    ('VAJKJSMHBJAY', 'Bowsprit', 'WA6304AF', 'Catamaran', 37.4, 11.4, 1988),
    ('RQM8G48QC9V5', 'Moonshadow', 'WA1510BR', 'Trawler', 23.6, 6.5, 2012),
    ('CLVKEJTW09U5', 'Riptide', 'WA8048DY', 'Sailboat', 33.6, 8.2, 2020),
    ('PLQ12GW3WSZK', 'Loose Change', 'WA8770CC', 'Sailboat', 38.0, 10.8, 1990),
    ('TBUXH2Y3DUVY', 'Ospreys Nest', 'WA9313GE', 'Pontoon', 29.1, 7.5, 2015),
    ('DJUQ3BTBMTVX', 'Blue Horizon', 'WA3972EU', 'Pontoon', 37.7, 11.0, 2010),
    ('CZSP025XLZVW', 'Coastal Drift', 'WA1861EF', 'Center Console', 19.1, 5.1, 1988),
    ('VQPTP8HY3HU7', 'Sandpiper', 'OR6053BH', 'Trawler', 36.8, 9.7, 2023),
    ('EJKWHA73MJ0Q', 'Meridian Star', 'WA6802CN', 'Pontoon', 28.1, 8.4, 1987),
    ('UPU12UH1BWL5', 'Free Spirit', 'WA2443PD', 'Powerboat', 32.3, 7.9, 2012),
    ('ZLHXLE8H98NY', 'Sunset Runner', 'WA4232FV', 'Powerboat', 39.0, 12.6, 1989),
    ('UUQWWK4E64VT', 'Windrunner', 'OR8565QB', 'Sailboat', 38.9, 10.4, 2008),
    ('VVS05C4NW68K', 'Compass Rose', 'WA2381SW', 'Powerboat', 31.3, 8.3, 1987);
...
CREATE TABLE Reservation (
    ...
    -- Added in 1.6.0. NOT NULL with a default of FALSE: every existing row
    -- needs an answer, and "no hookup" is the only answer that can be given
    -- honestly for seed data. The fee itself is Rate.ELECTRIC_MONTHLY and is
    -- already counted in monthlyRate; this column records only whether that
    -- fee is part of the total.
    electricalHookup BOOLEAN NOT NULL DEFAULT FALSE
    ...
    -- Added in 1.4.0. In the ERD from the start, missing from V1-0-0.
    reservationStatus VARCHAR(20) NOT NULL DEFAULT 'Active'
