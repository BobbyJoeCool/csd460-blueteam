...
-- File: MoffatBayMarinaDB_V1-4-0.sql
-- Purpose: Builds the complete database FROM SCRATCH at version 1.4.0 -
--          the state it is in today. This single file replaces the old
--          five-step path of running V1-0-0 and then each of the four
--          update scripts in order. Those five files still exist, under
--          Legacy/Week4/, as the record of how the schema got here; this
...
-- Date: 2026-09-07
-- =============================================================================
-- Run this script as a MySQL admin/root user (e.g. `mysql -u root -p < MoffatBayMarinaDB_V1-4-0.sql`).
...
-- indistinguishable from one built by running V1-0-0 and the four updates.
-- Dates are the dates each version was authored, not the date this script
-- runs - CURDATE() would claim all five happened today, which is untrue.
-- currentVersion.sql reads the last row, so this instance reports 1.4.0.
