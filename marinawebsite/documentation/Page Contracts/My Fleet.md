...
- [x] **Remove on a reserved boat.** The Remove button renders disabled, with a hint beside it ("Cancel this boat's reservation before removing it"). The server enforces this regardless — see [Removing a Boat](#removing-a-boat).
...
| Remove on a boat with an Active reservation | "This boat is reserved at Dock A, Slip 2 (A-02). Cancel that reservation before removing the boat." | Status popup on `myFleet.jsp` |
